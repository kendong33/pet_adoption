<x-app-layout>
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-extrabold text-gray-800">Add New Category</h1>
            <p class="text-gray-400 mt-1">Create a new classification for shelter pets</p>
        </div>

        {{-- Form --}}
        <form method="POST" action="{{ route('categories.store') }}" class="bg-white/70 backdrop-blur-sm rounded-3xl border-2 border-violet-100 shadow-sm p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="category_name" id="category_name" value="{{ old('category_name') }}" 
                           placeholder="e.g. Bird, Reptile, Fish..."
                           class="w-full rounded-2xl border-2 border-violet-100 bg-violet-50/50 text-sm text-gray-700 placeholder-gray-400 focus:ring-violet-300 focus:border-violet-300 px-4 py-3" required autofocus />
                    @error('category_name')
